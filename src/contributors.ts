interface Contributor {
  ...
  async function createContributorCard(contributor: Contributor): Promise<HTMLElement> {
    ...
    img.src = `https://github.com/${contributor.githubUsername}.png`
    img.alt = `${contributor.name}'s profile picture`
    ...
    username.href = `https://github.com/${contributor.githubUsername}`
    username.innerHTML = `<button class="Btn">
      ...
      <span class="text">${contributor.githubUsername}</span>
    </button>`
    username.className = 'Btn'
  ...
  export async function renderContributors(container: HTMLElement): Promise<void> {
    const grid = document.createElement('div')
    grid.className = 'contributors-grid'
    
    for (const contributor of contributors) {
      const card = await createContributorCard(contributor)
